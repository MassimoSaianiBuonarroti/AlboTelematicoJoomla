...
class AttoModel extends AdminModel
{
    ...
    /**
     * Salvataggio con gestione multipli allegati PDF (max 20, uno per volta)
    ...
    public function save($data)
    {
        ...
        if (!empty($file) && !empty($file['name'])) {
            // Controlla limite di 20
            if (count($attachments) >= 20) {
                $app->enqueueMessage('Hai già 20 allegati per questo atto. Non puoi aggiungerne altri.', 'warning');
            } else {
